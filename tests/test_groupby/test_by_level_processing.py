"""
Comprehensive tests for by/level argument processing in SeriesGroupBy and DataFrameGroupBy.
...
class TestDataFrameByLevelProcessing:
    ...
    def setup_method(self):
        ...
        self.df_mixed_cols = pd.DataFrame({
            ...
            pd.Timestamp('2024-01-01'): ['A', 'B', 'A', 'B', 'A', 'B'],  # Timestamp column name
            pd.Timedelta('1 day'): ['X', 'Y', 'X', 'Y', 'X', 'Y'],  # Timedelta column name
        })
        ...
        numeric_multi_index = pd.MultiIndex.from_tuples([
            ('A', 1), ('A', 2), ('B', 1), ('B', 2)
        ], names=[0, 1])  # Numeric level names
        self.df_numeric_levels = pd.DataFrame({
            'data': [1, 2, 3, 4]
        }, index=numeric_multi_index)
    ...
    def test_string_column_groupby(self):
        ...
        # Both should have same numeric columns (pandas-plus filters to numeric only)
        # pandas excludes the grouping column, pandas-plus includes it since it's numeric
    ...
    def test_callable_groupby(self):
        ...
        grouper_func = lambda x: len(str(x))
...
class TestSeriesByLevelProcessing:
    ...
    def setup_method(self):
        ...
        self.series_multi = pd.Series([1, 2, 3, 4, 5, 6], index=multi_index, name='values')
        
        # Series with numeric level names
        numeric_multi_index = pd.MultiIndex.from_tuples([
            ('A', 1), ('A', 2), ('B', 1), ('B', 2)
        ], names=[pd.Timestamp('2024-01-01'), 42])  # Timestamp and int level names
...
class TestErrorHandling:
    ...
    def test_invalid_column_name(self):
        ...
        with pytest.raises(KeyError, match="Column or index level 'nonexistent' not found"):
    ...
    def test_level_on_regular_index(self):
        ...
        with pytest.raises(ValueError, match="Level 'invalid' not valid for non-MultiIndex"):
    ...
    def test_array_length_mismatch(self):
        ...
        with pytest.raises(ValueError, match="Length of grouper \\(2\\) != length of DataFrame \\(4\\)"):
...
class TestEdgeCases:
    ...
    def setup_method(self):
        ...
        # DataFrame with duplicate column names (not supported by pandas but we should handle gracefully)
    ...
    def test_none_values_handling(self):
        ...
        with pytest.raises(ValueError, match="Must provide either 'by', 'level' or `grouper`"):
