...
class TestSeriesGroupBy:
    ...
    def setup_method(self):
        ...
        self.multi_data = pd.Series([10, 20, 30, 40, 50, 60], 
                                   index=self.multi_index, name='values')
    ...
    def test_combined_by_and_level_grouping(self):
        ...
        extra_grouper = pd.Series(['X', 'Y', 'X', 'Y', 'X', 'Y'], 
                                 index=self.multi_data.index, name='extra')
        
        gb = SeriesGroupBy(self.multi_data, by=extra_grouper, level=0)
        pandas_gb = self.multi_data.groupby([extra_grouper, 
                                           self.multi_data.index.get_level_values(0)])
...
class TestEdgeCases:
    ...
    def test_large_number_of_groups(self):
        ...
        groups = pd.Series([f'group_{i}' for i in range(n)])  # Each value in its own group
